...
1 + 1 + 1 + 1 + 1 + 1 + 0.9 + 1 + 1 + .75 = 9.65 out of 10 points possible
...
* if you use a log scale on the x-axis, this will space out the smaller population points while still maintaining the correct spacing between points
...
10. step 5 plot for variability in the time to fixation for each starting allele frequency. label the ticks and notate population size (0.5 pts for notes and labels)
  ...
  * axis labels are switched and pop size isn't notated --> -0.25
