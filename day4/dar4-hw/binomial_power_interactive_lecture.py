...
new_twodim_arr = numpy.zeros((len(probs), len(tosses)))   
##making the array exist
new2_twodim_arr = numpy.zeros((len(probs), len(tosses)))  

for i in range(len(probs)):
    for j in range(len(tosses)):
        power = run_experiment(probs[i],tosses[j])
        power2 = run_experiment(probs[i],tosses[j], correct_the_pvalues = True)
        new_twodim_arr[i,j] = power
        new2_twodim_arr[i,j] = power2
        
        
fig, ax = plt.subplots()
ax = sns.heatmap(new_twodim_arr, vmin = 0, vmax = 1, cmap='cool_r', xticklabels=tosses, yticklabels=probs)
ax.set_xlabel('Tosses')
ax.set_ylabel('Probabilities')
ax.set_title('Coin Flip Power')
plt.show()
fig.savefig( "2d_array" + ".png" )

fig, ax = plt.subplots()
ax = sns.heatmap(new2_twodim_arr, vmin = 0, vmax = 1, cmap='cool_r', xticklabels=tosses, yticklabels=probs)
ax.set_xlabel('Tosses')
ax.set_ylabel('Probabilities')
ax.set_title('Coin Flip Power')
plt.show()
fig.savefig( "2d_array_correction" + ".png" )
